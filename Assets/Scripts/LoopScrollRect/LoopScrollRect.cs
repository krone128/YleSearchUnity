...
namespace UnityEngine.UI
{
    ...
    {
        public event Action<int> ScrolledDownToIndex;
        
        [SerializeField]
        private string ListItemPrefabName;

        //==========LoopScrollRect==========

        //Total count, negative means INFINITE mode
        private int totalCount => dataSource?.Count ?? 0;
        ...
        public void ClearCells()
        {
            if (Application.isPlaying)
            {
                ...
                {
                    SG.ResourceManager.Instance.ReturnObjectToPool(content.GetChild(i).gameObject);
                }
            }
        }
        ...
        public void RefreshCells()
        {
            if (Application.isPlaying && this.isActiveAndEnabled)
            {
                ...
                for (int i = 0; i < content.childCount; i++)
                {
                    ...
                    {
                        InitListItem(content.GetChild(i).gameObject, itemTypeEnd);
                        itemTypeEnd++;
                    }
                    else
                    {
        ...
        public void RefillCellsFromEnd(int offset = 0)
        {
            ...
            {
                SG.ResourceManager.Instance.ReturnObjectToPool(m_Content.GetChild(i).gameObject);
            }
        ...
        public void RefillCells(int offset = 0, bool clearAll = true)
        {
            if (!Application.isPlaying)
                return;
        ...
        protected float DeleteItemAtStart()
        {
            ...
            for (int i = 0; i < contentConstraintCount; i++)
            {
                ...
                SG.ResourceManager.Instance.ReturnObjectToPool(oldItem.gameObject);
        ...
        {
            var nextItem = SG.ResourceManager.Instance.GetObjectFromPool(ListItemPrefabName, autoCreate:1).transform;
            InitListItem(nextItem.gameObject, itemIdx);
            
            nextItem.transform.SetParent(content, false);
            nextItem.gameObject.SetActive(true);
            
            ScrolledDownToIndex?.Invoke(itemIdx);
            
            return nextItem as RectTransform;
        }

        private void InitListItem(GameObject listItem, int dataIndex)
        {
            var dataReciever = listItem.GetComponent<IListItemDataReciever>();

            if (dataReciever == null)
            {
                Debug.LogError("Cant assign data - Data Reciever component not found");
                return;
            }
            
            dataReciever.Recieve(dataSource[dataIndex]);
        }
